...
rows_out = []
with open('/mnt/user-data/uploads/verification.csv', newline='') as f:
    r = csv.DictReader(f)
    for row in r:
        p = int(row['Pass'])
        if p not in (1, 2):
...
with open('/home/claude/work/golden.csv', 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=['pass','neuron','mac','bias','sum','tanh_in','tanh_out'])
    w.writeheader()
    w.writerows(rows_out)

print(f"Wrote {len(rows_out)} rows to golden.csv")
